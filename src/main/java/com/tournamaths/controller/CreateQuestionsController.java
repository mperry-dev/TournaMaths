 ...
 * Default propagation = REQUIRED ; if a transactional method calls another transactional method, the existing transaction continues.
 * It's important to use this propagation approach since doing dependency injection of Session - want to avoid them being used in the wrong transaction.
 * Rolling back by default is only for Runtime exceptions (not checked exceptions).
 * Because using Transactional, have container-managed transactions, so only need to rollback after unchecked exceptions:
...
public class CreateQuestionsController {
    ...
    public String all(Model model) {
        // Even though this is boilerplate, it is best to not autowire in since Hibernate does some validation when running getCriteriaBuilder - see:
        ...
        // This also ensures that the criteria builder is part of the current transaction context and persistence context.

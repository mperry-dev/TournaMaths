...
@Configuration
// only use this for production - locally rely on application-dev.properties and SpringBoot to generate Datasource for Autowiring
@Profile("prod")
public class ProdDBConfig {
  ...
  public DataSource dataSource() {
    ...
    // NOTE if a managed database password is used and the password changes, existing PostgreSQL
  ...
  private JSONObject getMasterSecret(DBInstance dbInstance) {
    ...
    GetSecretValueRequest request =
        GetSecretValueRequest.builder().secretId(masterSecretARN).build();
  ...
  private DBInstance getDBInstance() {
    // TODO = shift this into Java's try resource-setup syntax
    RdsClient rdsClient = RdsClient.builder().region(Region.of(region)).build();

    try {
      DescribeDbInstancesRequest request =
          DescribeDbInstancesRequest.builder().dbInstanceIdentifier("tournamath-db").build();
      ...
      if (dbInstances.size() != 1) {
        System.err.println(
            "Should have 1 database of identifier tournamath-db, but have " + dbInstances.size());
        System.exit(1);
      }
